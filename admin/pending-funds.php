...
if (isset($_GET['id'], $_GET['action']) && in_array($_GET['action'], ['approve', 'reject'])) {
    $fundId = new MongoDB\BSON\ObjectId($_GET['id']);
    $newStatus = $_GET['action'] === 'approve' ? 'approved' : 'rejected';
    $db->funds->updateOne(
        ['_id' => $fundId, 'status' => 'pending'],
        ['$set' => ['status' => $newStatus]]
    );
    header("Location: pending-funds.php");
    exit;
}

// Fetch only pending funds
$funds = $db->funds->find(['status' => 'pending'], ['sort' => ['created_at' => -1]]);
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    <meta charset="UTF-8">
    <title>Pending Funds | InvestHub Admin</title>
...
<body class="d-flex flex-column min-vh-100 bg-light">
    ...
    <div class="container py-4">
        <nav class="mb-4">
            <a href="pending-funds.php" class="btn btn-primary me-2">Pending Funds</a>
            <a href="funds-history.php" class="btn btn-outline-secondary">Funds History</a>
        </nav>
        <h1 class="mb-4">Pending Funds</h1>
                    ...
                    foreach ($funds as $fund): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="?id=<?= $fund['_id'] ?>&action=approve" class="btn btn-success btn-sm">Approve</a>
                                <a href="?id=<?= $fund['_id'] ?>&action=reject" class="btn btn-danger btn-sm">Reject</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
